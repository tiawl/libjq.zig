...
pub fn main() !void {
    ...
    if (c.jq_halted(jq) != 0) {
        ...
        if (c.jv_is_valid(exit_code) == 0) ret = ExitCode.JQ_OK else if (c.jv_get_kind(exit_code) == c.JV_KIND_NUMBER) ret = @enumFromInt(@as(i8, @intFromFloat(c.jv_number_value(exit_code)))) else ret = ExitCode.JQ_ERROR_UNKNOWN;
        ...
        if (c.jv_get_kind(msg) == c.JV_KIND_STRING) {
            std.debug.print("jq: error (at {s}): {s}\n", .{
                c.jv_string_value(input_pos),
                c.jv_string_value(msg),
            });
        } else {
